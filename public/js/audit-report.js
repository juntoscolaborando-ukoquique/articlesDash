...
// renderAuditReport() composes four independent section builders — one per
// report category — plus a closing OK/summary line. Each builder returns an
// element (or null when its category is empty) and knows nothing about the
// others, so a change to how duplicates render can't accidentally touch
// how orphaned markers render.

function buildDuplicateCard(group, verified) {
...
// ── A. Duplicados ──────────────────────────────────────────────────────
function buildDuplicatesSection(duplicates, verified) {
  ...
  for (const group of realDups) {
    sec.appendChild(buildDuplicateCard(group, verified));
  }
...
function buildWriteBacksSection(writeBacksMissing) {
  if (writeBacksMissing.length === 0) return null;
...
function buildOrphanedMarkersSection(orphanedMarkers) {
  if (orphanedMarkers.length === 0) return null;
...
// ── D. OK banner / summary line ───────────────────────────────────────
function buildOkOrSummary(report, realDuplicates) {
  const { writeBacksMissing, orphanedMarkers, ok } = report;
...
function renderAuditReport(report) {
  const { duplicates, writeBacksMissing, orphanedMarkers } = report;
  const realDuplicates = duplicates.filter((d) => !d.resolvedInSpip);

  const frag = document.createDocumentFragment();
  for (const section of [
    buildDuplicatesSection(duplicates, report.verified),
    buildWriteBacksSection(writeBacksMissing),
    buildOrphanedMarkersSection(orphanedMarkers),
  ]) {
    if (section) frag.appendChild(section);
  }
  frag.appendChild(buildOkOrSummary(report, realDuplicates));
